		...
		if(strtolower($this->auth_res) == 'success'){
			if(!empty($this->input->post('id')) && !empty($this->input->post('type'))){
				...
				if(empty($vdata[0]['id'])){
					...
					// -- comment up vote notification start here -- //
					if($up_vote>0 && !empty($up_vote)){
						$post_vpvote_ins = array(
							'user_id'=>$up_vote,
							'type'=>'comment_upvote',
							'to_id'=>$this->input->post('id'),
							'notification'=>'Someone upvotes to your comment.'
						);
						$this->db->insert('notifications', $post_vpvote_ins);
					}
					// -- comment up vote notification end here -- //
				}else{
					$vid = $vdata[0]['id'];
