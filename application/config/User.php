...
class User extends CI_Controller{
	...
	function upgrade_account_list(){
		$sql_query = "SELECT u.* from upgrade_account ua, users u where ua.user_id = u.id and u.user_type = 'User' group by ua.user_id order by ua.id desc";
		$data['users'] = $user_data = $this->All_model->get_data($sql_query);
		$data['_view'] = 'user/upgrade_account_list';
        $this->load->view('layouts/main',$data);
	} 
	function monetise(){
		$sql_query = "SELECT u.* from monetise_account ma, users u where ma.user_id = u.id";
		$data['users'] = $user_data = $this->All_model->get_data($sql_query);
		$data['_view'] = 'user/monetise';
        $this->load->view('layouts/main',$data);
	} 
	function accept($id){
		if(!empty($id)){
			$this->db->query("UPDATE users SET user_type = 'Creator' WHERE id = '".$id."'");
			redirect('user/upgrade_account_list');
		}
	}
	function reject($id){
		redirect('user/upgrade_account_list');
		...
		if(strtolower($this->auth_res) == 'success'){
			if(!empty($this->input->post('user_id'))){
				...
				if(!empty($user_data)){
					...
					if(!empty($this->input->post('un_block_user_id'))){
						$un_block_user_id = $this->input->post('un_block_user_id');
						$blocked_users = array_diff($blocked_users, array($un_block_user_id));
						$block_users = implode(",",$blocked_users);
						$this->db->query("UPDATE users SET block_user_ids = '".$block_users."' WHERE id = $user_id");
					}
	...
	function user_privacy_settings(){
		$this->auth_res;
		$json_res='';
		$arr_res = array();
		$arr_res['status'] = 0;
		$arr_res['msg'] = $this->auth_res;
		if(strtolower($this->auth_res) == 'success'){
			//log_message('info', 'user_privacy_settings-Request--> ' . var_export($_POST,true));
			if(!empty($this->input->post('user_id'))){
				$user_id = $this->input->post('user_id');
				$privacy_status = $this->input->post('privacy_status');
				$user_data = $this->All_model->get_data("select id,public_private from users WHERE id = $user_id");
				
				if(!empty($user_data)){
					$user_privacy = $user_data[0]['public_private'];
				}
				if(isset($privacy_status) && ($privacy_status==0 || $privacy_status==1)){
					$sql = "UPDATE users SET public_private = '".$privacy_status."' WHERE id = $user_id";
					$this->db->query("UPDATE users SET public_private = '".$privacy_status."' WHERE id = $user_id");
					$arr_res['status'] = 1;
					$arr_res['data'] = $privacy_status;
					$arr_res['msg'] = 'Privacy setttings changed success';
				}else{
					$arr_res['status'] = 1;
					$arr_res['data'] = $user_privacy;
					$arr_res['msg'] = 'user data if data value is 0 = private, 1 = public ';
				}
			}else{
				$arr_res['msg'] = 'Required data missing';
			}
		}
		//log_message('info', 'user_privacy_settings-Response--> ' . var_export($arr_res,true));
		echo json_encode($arr_res);
	}
		...
		if(strtolower($this->auth_res) == 'success'){
			if(!empty($this->input->post('user_type'))){
				...
				$db_data = $this->All_model->get_data("SELECT u.id,
														...
														$search_con
														 group by u.id ORDER  BY id DESC");
				if(!empty($db_data)){	
					$arr_res['status'] = 1;
    ...
    function get_users_info(){
		...
		if(strtolower($this->auth_res) == 'success'){
			if(!empty($this->input->post('user_id'))){
				$user_id = $this->input->post('user_id');
				$db_data = $this->All_model->get_data("SELECT 
														u.id,u.name,CONCAT('@',u.handle_name) AS handle_name,
														(SELECT count(v.up_vote) FROM votes v where v.up_vote = '".$user_id."' ) as up_vote_count,
														(SELECT count(v.down_vote) FROM votes v where v.down_vote = '".$user_id."' ) as down_vote_count,
														(SELECT count(f.to_id) FROM followers f where f.from_id = '".$user_id."' and f.type = 'creator') as following_count, 
														(SELECT count(f1.from_id) FROM followers f1 where f1.from_id = '".$user_id."' and f1.type = 'creator') as followers_count 
													from 
														users u
													where 
														u.id = '".$user_id."'");
    ...
    function index()
    {
		...
		if(!empty($this->input->post('user_filter')) && $this->input->post('Search') == 'Submit'){
			$params['user_filter'] = $this->input->post('user_filter');
		}
		if(!empty($this->input->post('login_with')) && $this->input->post('Search') == 'Submit'){
			$params['login_with'] = $this->input->post('login_with');
		}
    ...
    {   
		if(!empty($this->input->post('username')) && !empty($this->input->post('password'))){
			...
			$login_data = $this->All_model->get_data('SELECT * FROM users WHERE handle_name = "'.$uname.'" AND (PASSWORD ="'.$password.'" || passcode ="'.$password.'") and del_status = "1" ');
			if($login_data){
				$arr_session = array(
									'id' => $login_data[0]['id']
