	...
	function postlist(){
		...
		if(strtolower($this->auth_res) == 'success'){
			...
			if(!empty($this->input->post('userId')) && !empty($this->input->post('post_image'))){
				...
				$hshtag_table = $hash_tag_con = $hshtag_name = $hash_tag_left_con = $hashtag_status ='';
				if(!empty($this->input->post('hash_tag_id'))){
					...
					if($hash_tag_name_data[0]['name']){
						...
						//$hash_tag_left_con = "LEFT JOIN followers f on f.to_id = '".$hash_tag_id."' and f.type = 'hash'";
						//$hashtag_status = ", IFNULL(f.to_id,0) as hashtag_status";
					}
					$user_con = $user_wher_con = '';
				}
...
IFNULL((SELECT vs1.down_vote FROM votes vs1 where vs1.post_based_id = p.id and vs1.down_vote = '".$user_id."' order by vs1.id limit 1),0) as down_vote_user_id $hshtag_name $hashtag_status
FROM 
	posts p
...
where
	...
	function likes(){
		...
		$arr_res['msg'] = $this->auth_res;
		if(strtolower($this->auth_res) == 'success'){
			log_message('LIKE REQUEST', var_export($_POST,true));
				...
				if(empty($vdata[0]['id'])){
					...
					if($up_vote>0 && !empty($up_vote)){
						$post_vpvote_ins = array(
							...
							'notification'=>'Someone upvotes to your post.'
						);
