		...
		if(strtolower($this->auth_res) == 'success'){
			if(!empty($this->input->post('user_id'))
				...
				if($user_data[0]['public_private'] == 1 && strtolower($this->input->post('type')) != 'hash'){
					...
					// --Insert Notifications start here -- //
					if(!empty($ins_status)){
						$arrNotifInsert = array(
							"user_id"=>$this->input->post('user_id'),
							"type"=>'follow',
							"to_id"=>$this->input->post('to_id'),
							"notification"=>'Someone following you',
						);
						$ins_status = $this->db->insert('notifications',$arrNotifInsert);
					}
					// --Insert Notifications end here -- //
